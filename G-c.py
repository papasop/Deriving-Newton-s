# ================== One-Click: Σ-form & κ-form (comp+DC) on a public κ-FITS ==================
# 依赖：Colab 通常自带 astropy/scipy/matplotlib；若缺少会自动安装
import subprocess, sys, importlib
def _pip(pkgs): subprocess.check_call([sys.executable, "-m", "pip", "install", "-q"] + pkgs)
for mod in ["astropy","scipy","matplotlib","numpy"]:
...
# -------------------- 配置（改这里即可切换数据/参数） --------------------
# 1) 公开 κ-FITS 下载地址：默认 HST Frontier Fields Abell 2744（CATS v4）
...
#    若要换 Euclid Q1/别的 2D κ 图：把上面 URL 替换成公开可下载的 κ-FITS

# 2) 透镜&源红移（Σ-口径用；κ-口径与红移无关）
z_d = 0.308   # Abell 2744
z_s = 2.0     # 示例：源像有效红移（按需要修改）
...
apod_frac       = 0.10    # 边缘加窗（0.04~0.12）
ring_count      = 24      # 同心环数
ring_inner_frac = 0.10    # 半径内侧（相对 min(nx,ny)）
ring_outer_frac = 0.35    # 半径外侧
M_samples       = 4096    # 圆周采样
bg_annulus_frac = 0.08    # κ-补偿的背景环厚度
gap_frac        = 0.01    # 盘/背景环间隙
win_thr         = 1e-3    # 仅在 win>thr 处积分
...
def save_summary(prefix, summary_dict, per_ring_dict, out_dir=OUT_DIR):
    ...
    print("Saved CSV:", sp); print("Saved per-ring CSV:", prp)
...
for Rpix in radii:
    ...
    dΩ = pix_rad**2
    Marea = (Sigma_s[inside] * dΩ).sum()
    F_vol = (LapPhi_n[inside] * dΩ).sum()
...
# DC-consistent：面积端减去与 FFT 去掉相同的 DC 常数
Sigma_eff   = Sigma_s * win
Rbar_sigma  = Rw_sigma.mean()
Sigma0_dc   = - Rbar_sigma / k_th
Fs_dc, Ms_dc, rels_dc = [], [], []
for Rpix in radii:
    F_line = ring_integral_normal(Phi_n_k, grids_n, Rpix, pix_rad, M_samples)
    inside, _, _, _ = disk_mask(ny, nx, Rpix)
...
# DC-consistent（面积端减去与 FFT 去除相同 DC）
Rbar_k   = R_kappa_w.mean()
kappa0_dc= Rbar_k / 2.0
Fs_dc, Ms_dc, rels_dc = [], [], []
for Rpix in radii:
    F_line = ring_integral_normal(Psi_k, grids_k, Rpix, pix_rad, M_samples)
    inside, _, _, _ = disk_mask(ny, nx, Rpix)
